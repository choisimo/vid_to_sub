...
선언된 Textual 최소 지원선은 `0.80.x`이며, CI에서는 `textual==0.80.1`과
`requirements.txt`가 허용하는 최신 버전을 함께 검증합니다.
...
나중 번역을 위해 1단계 전사만 수행하고 stage artifact 저장:

```bash
python vid_to_sub.py /path/to/videos --stage1-only --translate-to ko
```

이 명령은 각 원본 파일 옆에 `movie.srt`와 `movie.stage1.json`을 함께 만듭니다.
artifact에 `target_lang`가 이미 기록되어 있으면 나중에 `--translate-to`를 다시
주지 않아도 2단계 번역만 재실행할 수 있고, `--translate-to <lang>`로 저장된 값을
override할 수도 있습니다. 재실행 시에는 현재 원본 파일을 artifact의
`source_fingerprint`와 대조합니다. 의도적으로 이 검증이나 suspicious 체크를
우회하려는 경우에만 `--force-translate`를 사용하세요.

번역 API를 나중에 붙일 때는 재전사 없이 artifact만으로 2단계를 다시 실행할 수 있습니다:

```bash
python vid_to_sub.py --translate-from-artifact /path/to/movie.stage1.json
```

artifact 안에 이미 완료된 번역 기록이 있어도 강제로 다시 번역하려면:
...
- `VID_TO_SUB_DB_PATH`
  설정, SSH 프로필, 작업 이력을 저장하는 SQLite DB 경로를 직접 지정합니다.

기본 DB 경로 정책:

- 새 설치에서는 `XDG_STATE_HOME`이 설정돼 있으면 `$XDG_STATE_HOME/vid_to_sub/vid_to_sub.db`를 사용합니다.
- 그렇지 않으면 `~/.local/state/vid_to_sub/vid_to_sub.db`를 사용합니다.
- 프로젝트 루트에 기존 legacy DB인 `./vid_to_sub.db`가 이미 있으면, 하위 호환성을 위해 그 경로를 계속 사용합니다.

지정한 DB 경로가 중첩 디렉터리를 포함하면 상위 디렉터리는 자동으로 생성됩니다.

## 분산 실행


TUI는 SSH 리소스 프로필 기반 분산 모드를 지원합니다. 원격 실행기는 먼저
`Settings -> SSH Connections`에 저장해 두고, 기존 `Settings -> Remote Resources`
JSON은 fallback/import 용도로만 쓰는 것을 권장합니다. 이름이 겹치면 저장된
SSH connection 항목이 우선합니다.

실행 전에는 `Transcribe` 탭의 `Execution -> Mode`를 `distributed`로 바꾸세요.
분산 1단계가 원격 호스트에서 끝나면 TUI는 `.stage1.json` artifact를 로컬
파일시스템으로 가져와 기존 로컬 2단계 후속 실행을 자동으로 이어서 시도합니다.
이 fetch/remap 단계가 실패하면 저장된 SSH connection과 `path_map`을 점검하거나,
artifact를 직접 가져온 뒤 `--translate-from-artifact`로 수동 재실행하세요.
...
- `Settings` 탭에서 현재 비민감 설정을 `.env`로 내보낼 수 있고, API key는 세션/환경 변수 전용으로 남기며 export 대상에서 제외됩니다.

## 기존 자막 재사용 및 자동 후보 검색

새 기능은 전사 전에 자막 상태를 먼저 확인합니다. 상태 전이는 다음과 같습니다.

1. 비디오 발견
2. 로컬 sidecar 자막, 내장 텍스트 자막 스트림, 외부 제공자 후보 검색
3. 후보 점수화 및 다운로드 가능 목록 출력
4. 임계값 이상 후보가 있으면 기존 자막을 파싱하고 싱크를 보정
5. 기존 자막으로 원본 출력과 `.stage1.json` artifact 생성
6. `--translate-to`가 있으면 기존 stage 2 번역 파이프라인으로 이어짐
7. 후보가 없거나 점수가 낮으면 기존 Whisper 전사 경로로 롤백

후보만 확인하려면 다음처럼 실행합니다.
...
싱크 보정 방식은 `--subtitle-sync-mode off|duration|asr`로 선택합니다. `duration`은 명백한 범위 초과 시간을 비디오 길이에 맞춰 보정합니다. `asr`은 전사를 최종 자막 생성용으로 쓰지 않고, 기존 자막의 시간축을 맞추기 위한 기준 transcript로만 사용한 뒤 텍스트 anchor 기반 offset/scale을 추정합니다.

외부 제공자는 현재 OpenSubtitles와 SubDL adapter가 포함되어 있습니다. API 키가 없으면 해당 provider는 조용히 건너뛰고 로컬/내장 후보만 사용합니다.

```bash
export VID_TO_SUB_OPENSUBTITLES_API_KEY=...
export VID_TO_SUB_OPENSUBTITLES_USERNAME=...      # 선택
export VID_TO_SUB_OPENSUBTITLES_PASSWORD=...      # 선택
export VID_TO_SUB_SUBDL_API_KEY=...               # 선택
```
