...
주지 않아도 2단계 번역만 재실행할 수 있습니다. 저장된 대상 언어가 없으면
재실행 시 `--translate-to <lang>`를 함께 넘기세요.
...
자동 2단계 후속 실행은 로컬 파일시스템에서 보이는 `.stage1.json`만 수집합니다.
분산 1단계 결과물이 원격 호스트에만 남아 있다면 artifact를 로컬로 가져온 뒤
`--translate-from-artifact`로 수동 재실행해야 합니다.
